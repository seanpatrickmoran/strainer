...
#    try:
#        cursor = connection.execute("CREATE TABLE imag(name, coordinates, numpyarr, viewing_vmax, dimensions, hic_path, PUB_ID, resolution, meta)")
...
#            cursor.executemany("INSERT INTO imag VALUES(:name, :coordinates, :numpyarr, :viewing_vmax, :dimensions, :hic_path, :PUB_ID, :resolution, :meta)", data)
#            print(f"success")
...
#            connection.close()
#            
#    if not all(i in kwargs for i in ["image_np_dict","prefix_name","hic_path","PUB_ID","resolution"]):
#        raise Exception("\"image_np_dict\",\"prefix_name\",\"hic_path\",\"PUB_ID\", or \"resolution\" not in kwargs")
#        return
#    
#    data = []
...
#        data += [C3Image3(f"{prefix_name}_#{i}", *[fields[k] for k in fields.keys()],focus['numpy_window'].shape[0],resolution,hic_path,PUB_ID,genome="hg38",).entity]
#    _write_db(data)
#    
#
#def _writeSingularToTable(dbPATH,timeout,**kwargs):
#    data = (
#            kwargs["C3Image2"].entity
#            )
#    def _write_db(data):
#        connection,cursor=call(dbPATH,timeout)
#        try:
...
#            cursor.close()
#            connection.close()
#    if "C3Image2" not in kwargs:
#        raise Exception("need \"C3Image2\": C3Image2 object in kwargs")
#    _write_db(kwargs["C3Image2"])
#
#
...
#        raise Exception("need query \'hic_path\' in kwargs")
...
def _writeManyToTable(dbPATH,timeout,**kwargs):
    def _write_db(data):
        ...
        try:
            data = tuple(x for x in data)
            cursor.executemany("INSERT INTO imag VALUES(:name, :dataset, :condition, :coordinates, :numpyarr, :viewing_vmax, :dimensions, :hic_path, :PUB_ID, :resolution, :norm, :meta)", data)
            print(f"success")
        except Exception as e:
            print(e)
    ...
    for i in image_np_dict.keys():
        ...
        focus = image_np_dict[i]
        fields = extractor(focus, 'original coordinates', 'numpy_window', 'viewing_vmax',)
        data += [C3Image4(f"{prefix_name}_#{i}", *[fields[k] for k in fields.keys()],focus['numpy_window'].shape[0],resolution,hic_path,PUB_ID,genome="hg38", dataset=dataset,condition=condition,norm=norm).entity]
    _write_db(data)
